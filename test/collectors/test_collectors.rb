...
class TestCollectors < Test::Unit::TestCase
  def setup
    @collectors = Seacrest::Collectors
  ...
  def test_see_if_class_exists
    assert Seacrest::Collectors.const_defined?("CSSCollector"), "Should return true for Collectors::CSSCollector existing"
    assert ! Seacrest::Collectors.const_defined?("MPTCollector"), "There's currently not such filetype as .mpt (as far as I know), so this shouldn't exist."
  end
